...
<body>
  ...
  app.get('/api/payment/config', isAuthenticated, async (req, res) => {
    try {
      ...
      // Validate that credentials don't contain error messages
      if (apiLoginId.includes('Error') || clientKey.includes('Error')) {
        console.error('Invalid credentials detected - contain error messages');
        return res.json({ 
          success: false, 
...
<container_status>
  ...
  app.post('/api/ai-support', async (req: any, res) => {
    try {
      ...
      const diagReply = process.env.NODE_ENV !== 'production'
        ? `AI error: ${errMsg}`
        : 'Our AI support is temporarily unavailable. Please try again shortly or contact FreightClear directly at freightclear.com.';
      res.status(500).json({
